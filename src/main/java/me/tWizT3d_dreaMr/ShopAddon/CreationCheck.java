    ...
    public MatchType test(ItemStack i, double price, int amount, ShopType st) {
        for (Filter f : filters) {
            ...
            if (f.ListType().equalsIgnoreCase("PRICE")) {
                final var auctionHouseOptional = Plugin.getPlugin(Plugin.class)
                        .getAuctionHousePriceManager();
                if(auctionHouseOptional.isPresent()) {
                    final var priceManager = auctionHouseOptional.get().get();
                    if(priceManager.getMaxPrice(i) > price)
                        return new MatchType("pricemax", f);
                    if(priceManager.getMinPrice(i) < price)
